...
def _meson_overrides_impl(module_ctx):
    overrides = {}
    ...
    _overrides_repo(
        name = "meson_overrides",
        content = json.encode(overrides),
    )

meson = module_extension(
    implementation = _meson_overrides_impl,
    tag_classes = dict(
        override = tag_class(
            doc = "Specify meson-specific package overrides.",
            attrs = MESON_OVERRIDE_ATTRS,
        ),
    ),
)
