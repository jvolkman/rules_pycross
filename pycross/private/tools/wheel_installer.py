...
class FilteredWheelFile(WheelFile):
    def __init__(self, f: zipfile.ZipFile, install_exclude_globs: List[str]) -> None:
        super().__init__(f)
        self.install_exclude_globs = install_exclude_globs

    @classmethod
    @contextmanager
    def open(cls, path: "os.PathLike[str]", install_exclude_globs: List[str]) -> Iterator["FilteredWheelFile"]:
    ...
    def should_install(self, filename: str) -> bool:
        for install_exclude_glob in self.install_exclude_globs:
...
def main(args: Any) -> None:
    ...
    try:
        with FilteredWheelFile.open(link_path, args.install_exclude_globs) as source:
            install(
                source=source,
