...
class PackageTarget:
    ...
    def _render_pkg(self) -> str:
        lines = [
            ...
            ind(f'package_name = "{self.package.key.name}",'),
            ind(f'package_version = "{self.package.key.version}",'),
        ]
        if self._has_runtime_deps:
...
def gen_load_statements(imports: Set[str], pycross_repo: str) -> List[str]:
    ...
    sorted_files = sorted(
        load_statement_groups, key=lambda f: (1 if f.startswith("@@") else (0 if f.startswith("@") else 2), f)
    )
