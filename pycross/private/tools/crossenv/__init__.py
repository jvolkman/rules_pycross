...
def guess_target_platform(host_gnu_type: str) -> str:
    ...
    if len(host) == 4:  # i.e., aarch64-unknown-linux-gnu
        ...
        raise ValueError(f"Cannot determine target platform from HOST_GNU_TYPE: {host_gnu_type}")
...
def guess_sysconfig_platform(uname: Uname, target_platform: str, macosx_deployment_target: Optional[str]) -> str:
...
def write_sysconfig_data(sysconfig_data_path: Path, sysconfig_vars: Dict[str, Any]) -> None:
