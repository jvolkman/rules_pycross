# Derived from rules_foreign_cc (https://github.com/bazel-contrib/rules_foreign_cc)
# Copyright The rules_foreign_cc Authors. Licensed under Apache License 2.0.
# Modifications: adapted for rules_pycross build actions with pycross-specific
# settings labels and build-system environment variable injection.
...
def _setting(size, resource, mode):
    ...
    if mode == "key":
        ...
        return "//pycross/settings:size_" + short_name
    else:
        fail("unknown mode", mode)

def create_resource_set_settings():
    """Declares the resource_set build settings (int_flag / string_flag targets)."""
    int_flag(
    ...
    for size, cfg in _SIZES.items():
        ...
        for resource in ["cpu", "mem"]:
            ...
            int_flag(
                ...
                build_setting_default = cfg[resource],
                visibility = ["//visibility:public"],
            )
...
@rules_pycross//pycross/settings:parallelism_overcommit.

Each size maps to a cpu and mem value that can be overridden per-size.
See @rules_pycross//pycross/settings:size_{size}_{cpu|mem}.
...
""",
    ...
    "_parallelism_overcommit": attr.label(
        default = "//pycross/settings:" + _PARALLELISM_OVERCOMMIT_SETTING,
...
def get_resource_set(attr):
    ...
    overcommit = attr._parallelism_overcommit[BuildSettingInfo].value if allow_cpu_overcommit else 0
    parallelism = actual_cpu + overcommit if actual_cpu > 0 else 0

    return struct(
        resource_set = resource_set,
        cpu = actual_cpu,
        mem = actual_mem,
        allow_cpu_overcommit = allow_cpu_overcommit,
