...
def main() -> None:
    ...
    parser.add_argument("--in-wheelhouse", help="Path to the input wheelhouse directory.")
    parser.add_argument(
        "--out-wheelhouse", required=True, help="Directory where the transform tool writes the output wheel."
    )
    ...
    if args.in_wheelhouse:
        whl_files = glob.glob(os.path.join(args.in_wheelhouse, "*.whl"))
        if not whl_files:
            print("ERROR: No .whl file found in wheel directory: " + args.in_wheelhouse, file=sys.stderr)
            sys.exit(1)
        wheel_file = whl_files[0]
    else:
        print("ERROR: --in-wheelhouse is required", file=sys.stderr)
        sys.exit(1)

    # Build the environment for the transform tool.
    env = os.environ.copy()
    env["PYCROSS_WHEEL_FILE"] = wheel_file
    # The transform tool should write its output to args.out_wheelhouse
    os.makedirs(args.out_wheelhouse, exist_ok=True)
    env["PYCROSS_WHEEL_OUTPUT_ROOT"] = args.out_wheelhouse
    ...
    output_dir = Path(args.out_wheelhouse)
